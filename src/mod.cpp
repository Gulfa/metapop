...
[[cpp11::register]]
SEXP dust_cpu_mod_alloc(cpp11::list r_pars, bool pars_multi, cpp11::sexp r_time,
                         cpp11::sexp r_n_particles, int n_threads,
                         cpp11::sexp r_seed, bool deterministic,
                         cpp11::sexp gpu_config, cpp11::sexp ode_control);

[[cpp11::register]]
SEXP dust_cpu_mod_run(SEXP ptr, cpp11::sexp r_time_end);
...
[[cpp11::register]]
SEXP dust_cpu_mod_update_state(SEXP ptr, SEXP r_pars, SEXP r_state,
...
[[cpp11::register]]
SEXP dust_cpu_mod_time(SEXP ptr);
...
[[cpp11::register]]
SEXP dust_cpu_mod_filter(SEXP ptr, SEXP time_end,
...
public:
  ...
  mod(const dust::pars_type<mod>& pars) :
    shared(pars.shared), internal(pars.internal) {
  }
  size_t size() {
    return shared->dim_A + shared->dim_B_D + shared->dim_B_D_H + shared->dim_B_D_ICU + shared->dim_D + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_ICU_P + shared->dim_ICU_R + shared->dim_P + shared->dim_R + shared->dim_S + shared->dim_tot_hosp + shared->dim_tot_infected + shared->dim_tot_resp + 1;
  }
  std::vector<real_type> initial(size_t step) {
    std::vector<real_type> state(shared->dim_A + shared->dim_B_D + shared->dim_B_D_H + shared->dim_B_D_ICU + shared->dim_D + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_ICU_P + shared->dim_ICU_R + shared->dim_P + shared->dim_R + shared->dim_S + shared->dim_tot_hosp + shared->dim_tot_infected + shared->dim_tot_resp + 1);
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    for (int i = 1; i <= shared->dim_I_1; ++i) {
      for (int j = 1; j <= shared->dim_I_2; ++j) {
        for (int k = 1; k <= shared->dim_I_3; ++k) {
          state_next[shared->offset_variable_I + i - 1 + shared->dim_I_1 * (j - 1) + shared->dim_I_12 * (k - 1)] = I[shared->dim_I_12 * (k - 1) + shared->dim_I_1 * (j - 1) + i - 1] + internal.n_PI[shared->dim_n_PI_12 * (k - 1) + shared->dim_n_PI_1 * (j - 1) + i - 1] - internal.n_I[shared->dim_n_I_12 * (k - 1) + shared->dim_n_I_1 * (j - 1) + i - 1];
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_S_1; ++i) {
      for (int j = 1; j <= shared->dim_S_2; ++j) {
        state_next[1 + i - 1 + shared->dim_S_1 * (j - 1)] = S[shared->dim_S_1 * (j - 1) + i - 1] - odin_sum3<real_type>(internal.n_SE.data(), i - 1, i, j - 1, j, 0, shared->dim_n_SE_3, shared->dim_n_SE_1, shared->dim_n_SE_12);
      }
    }
...
namespace dust {
template<>
dust::pars_type<mod> dust_pars<mod>(cpp11::list user) {
  typedef typename mod::real_type real_type;
  auto shared = std::make_shared<mod::shared_type>();
  mod::internal_type internal;
  ...
  shared->p_waning = 1 - std::exp(- shared->dt / (real_type) shared->waning_immunity);
  ...
  shared->offset_variable_A = shared->dim_Ea + shared->dim_Es + shared->dim_I + shared->dim_P + shared->dim_S + 1;
  shared->offset_variable_B_D = shared->dim_A + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_ICU_P + shared->dim_ICU_R + shared->dim_P + shared->dim_S + 1;
  shared->offset_variable_B_D_H = shared->dim_A + shared->dim_B_D + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_ICU_P + shared->dim_ICU_R + shared->dim_P + shared->dim_S + 1;
  shared->offset_variable_B_D_ICU = shared->dim_A + shared->dim_B_D + shared->dim_B_D_H + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_ICU_P + shared->dim_ICU_R + shared->dim_P + shared->dim_S + 1;
  shared->offset_variable_D = shared->dim_A + shared->dim_B_D + shared->dim_B_D_H + shared->dim_B_D_ICU + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_ICU_P + shared->dim_ICU_R + shared->dim_P + shared->dim_R + shared->dim_S + 1;
  shared->offset_variable_Ea = shared->dim_S + 1;
  shared->offset_variable_Es = shared->dim_Ea + shared->dim_S + 1;
  shared->offset_variable_H = shared->dim_A + shared->dim_Ea + shared->dim_Es + shared->dim_I + shared->dim_P + shared->dim_S + 1;
  shared->offset_variable_I = shared->dim_Ea + shared->dim_Es + shared->dim_P + shared->dim_S + 1;
  shared->offset_variable_ICU_H = shared->dim_A + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_P + shared->dim_S + 1;
  shared->offset_variable_ICU_P = shared->dim_A + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_ICU_R + shared->dim_P + shared->dim_S + 1;
  shared->offset_variable_ICU_R = shared->dim_A + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_P + shared->dim_S + 1;
  shared->offset_variable_P = shared->dim_Ea + shared->dim_Es + shared->dim_S + 1;
  shared->offset_variable_R = shared->dim_A + shared->dim_B_D + shared->dim_B_D_H + shared->dim_B_D_ICU + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_ICU_P + shared->dim_ICU_R + shared->dim_P + shared->dim_S + 1;
  shared->offset_variable_tot_hosp = shared->dim_A + shared->dim_B_D + shared->dim_B_D_H + shared->dim_B_D_ICU + shared->dim_D + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_ICU_P + shared->dim_ICU_R + shared->dim_P + shared->dim_R + shared->dim_S + shared->dim_tot_infected + 1;
  shared->offset_variable_tot_infected = shared->dim_A + shared->dim_B_D + shared->dim_B_D_H + shared->dim_B_D_ICU + shared->dim_D + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_ICU_P + shared->dim_ICU_R + shared->dim_P + shared->dim_R + shared->dim_S + 1;
  shared->offset_variable_tot_resp = shared->dim_A + shared->dim_B_D + shared->dim_B_D_H + shared->dim_B_D_ICU + shared->dim_D + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_ICU_P + shared->dim_ICU_R + shared->dim_P + shared->dim_R + shared->dim_S + shared->dim_tot_hosp + shared->dim_tot_infected + 1;
  ...
  return dust::pars_type<mod>(shared, internal);
}
template <>
cpp11::sexp dust_info<mod>(const dust::pars_type<mod>& pars) {
  const mod::internal_type internal = pars.internal;
  const std::shared_ptr<const mod::shared_type> shared = pars.shared;
  cpp11::writable::strings nms({"time", "S", "Ea", "Es", "P", "I", "A", "H", "ICU_H", "ICU_R", "ICU_P", "B_D", "B_D_H", "B_D_ICU", "R", "D", "tot_infected", "tot_hosp", "tot_resp"});
  cpp11::writable::list dim(19);
  dim[0] = cpp11::writable::integers({1});
  dim[1] = cpp11::writable::integers({shared->dim_S_1, shared->dim_S_2});
  dim[2] = cpp11::writable::integers({shared->dim_Ea_1, shared->dim_Ea_2, shared->dim_Ea_3});
  dim[3] = cpp11::writable::integers({shared->dim_Es_1, shared->dim_Es_2, shared->dim_Es_3});
  dim[4] = cpp11::writable::integers({shared->dim_P_1, shared->dim_P_2, shared->dim_P_3});
  dim[5] = cpp11::writable::integers({shared->dim_I_1, shared->dim_I_2, shared->dim_I_3});
  dim[6] = cpp11::writable::integers({shared->dim_A_1, shared->dim_A_2, shared->dim_A_3});
  dim[7] = cpp11::writable::integers({shared->dim_H_1, shared->dim_H_2, shared->dim_H_3});
  dim[8] = cpp11::writable::integers({shared->dim_ICU_H_1, shared->dim_ICU_H_2, shared->dim_ICU_H_3});
  dim[9] = cpp11::writable::integers({shared->dim_ICU_R_1, shared->dim_ICU_R_2, shared->dim_ICU_R_3});
  dim[10] = cpp11::writable::integers({shared->dim_ICU_P_1, shared->dim_ICU_P_2, shared->dim_ICU_P_3});
  dim[11] = cpp11::writable::integers({shared->dim_B_D_1, shared->dim_B_D_2, shared->dim_B_D_3});
  dim[12] = cpp11::writable::integers({shared->dim_B_D_H_1, shared->dim_B_D_H_2, shared->dim_B_D_H_3});
  dim[13] = cpp11::writable::integers({shared->dim_B_D_ICU_1, shared->dim_B_D_ICU_2, shared->dim_B_D_ICU_3});
  dim[14] = cpp11::writable::integers({shared->dim_R_1, shared->dim_R_2, shared->dim_R_3});
  dim[15] = cpp11::writable::integers({shared->dim_D_1, shared->dim_D_2, shared->dim_D_3});
  dim[16] = cpp11::writable::integers({shared->dim_tot_infected_1, shared->dim_tot_infected_2, shared->dim_tot_infected_3});
  dim[17] = cpp11::writable::integers({shared->dim_tot_hosp_1, shared->dim_tot_hosp_2, shared->dim_tot_hosp_3});
  dim[18] = cpp11::writable::integers({shared->dim_tot_resp_1, shared->dim_tot_resp_2, shared->dim_tot_resp_3});
  dim.names() = nms;
  cpp11::writable::list index(19);
  index[0] = cpp11::writable::integers({1});
  index[1] = integer_sequence(2, shared->dim_S);
  index[2] = integer_sequence(shared->offset_variable_Ea + 1, shared->dim_Ea);
  index[3] = integer_sequence(shared->offset_variable_Es + 1, shared->dim_Es);
  index[4] = integer_sequence(shared->offset_variable_P + 1, shared->dim_P);
  index[5] = integer_sequence(shared->offset_variable_I + 1, shared->dim_I);
  index[6] = integer_sequence(shared->offset_variable_A + 1, shared->dim_A);
  index[7] = integer_sequence(shared->offset_variable_H + 1, shared->dim_H);
  index[8] = integer_sequence(shared->offset_variable_ICU_H + 1, shared->dim_ICU_H);
  index[9] = integer_sequence(shared->offset_variable_ICU_R + 1, shared->dim_ICU_R);
  index[10] = integer_sequence(shared->offset_variable_ICU_P + 1, shared->dim_ICU_P);
  index[11] = integer_sequence(shared->offset_variable_B_D + 1, shared->dim_B_D);
  index[12] = integer_sequence(shared->offset_variable_B_D_H + 1, shared->dim_B_D_H);
  index[13] = integer_sequence(shared->offset_variable_B_D_ICU + 1, shared->dim_B_D_ICU);
  ...
  index[16] = integer_sequence(shared->offset_variable_tot_infected + 1, shared->dim_tot_infected);
  index[17] = integer_sequence(shared->offset_variable_tot_hosp + 1, shared->dim_tot_hosp);
  index[18] = integer_sequence(shared->offset_variable_tot_resp + 1, shared->dim_tot_resp);
  index.names() = nms;
...
cpp11::sexp dust_mod_capabilities() {
  return dust::r::dust_capabilities<mod>();
}

cpp11::sexp dust_mod_gpu_info() {
  return dust::gpu::r::gpu_info();
}
using model_cpu = dust::dust_cpu<mod>;

SEXP dust_cpu_mod_alloc(cpp11::list r_pars, bool pars_multi, cpp11::sexp r_time,
                             cpp11::sexp r_n_particles, int n_threads,
                             cpp11::sexp r_seed, bool deterministic,
                             cpp11::sexp gpu_config, cpp11::sexp ode_control) {
  return dust::r::dust_cpu_alloc<mod>(r_pars, pars_multi, r_time, r_n_particles,
...
SEXP dust_cpu_mod_simulate(SEXP ptr, cpp11::sexp r_time_end) {
