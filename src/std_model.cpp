...
[[cpp11::register]]
SEXP dust_cpu_std_model_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
...
[[cpp11::register]]
SEXP dust_cpu_std_model_update_state(SEXP ptr, SEXP r_pars, SEXP r_state,
...
[[cpp11::register]]
SEXP dust_cpu_std_model_filter(SEXP ptr, SEXP step_end,
...
public:
  ...
  std_model(const dust::pars_type<std_model>& pars) :
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    real_type n_EI_r = dust::random::binomial<real_type>(rng_state, E_r, shared->p_ER);
    real_type n_IR_r = dust::random::binomial<real_type>(rng_state, I_r, shared->p_IR);
    for (int i = 1; i <= shared->dim_n_EI_msm; ++i) {
      internal.n_EI_msm[i - 1] = dust::random::binomial<real_type>(rng_state, E_msm[i - 1], shared->p_ER);
    }
    ...
    for (int i = 1; i <= shared->dim_S_msm; ++i) {
      state_next[8 + i - 1] = S_msm[i - 1] - odin_sum1<real_type>(internal.n_SI_msm.data(), i - 1, i);
    }
...
namespace dust {
template<>
dust::pars_type<std_model> dust_pars<std_model>(cpp11::list user) {
  typedef typename std_model::real_type real_type;
  auto shared = std::make_shared<std_model::shared_type>();
  std_model::internal_type internal;
  ...
  shared->p_ER = 1 - std::exp(- 1 / (real_type) shared->latent_periode * shared->dt);
  ...
  return dust::pars_type<std_model>(shared, internal);
}
template <>
cpp11::sexp dust_info<std_model>(const dust::pars_type<std_model>& pars) {
  const std_model::internal_type internal = pars.internal;
  const std::shared_ptr<const std_model::shared_type> shared = pars.shared;
...
cpp11::sexp dust_std_model_capabilities() {
  return dust::r::dust_capabilities<std_model>();
}

cpp11::sexp dust_std_model_gpu_info() {
  return dust::gpu::r::gpu_info();
}
using model_cpu = dust::dust_cpu<std_model>;

SEXP dust_cpu_std_model_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
                             cpp11::sexp r_n_particles, size_t n_threads,
                             cpp11::sexp r_seed, bool deterministic,
                             cpp11::sexp gpu_config) {
  return dust::r::dust_cpu_alloc<std_model>(r_pars, pars_multi, step, r_n_particles,
...
SEXP dust_cpu_std_model_set_data(SEXP ptr, cpp11::list data,
                                       bool shared) {
