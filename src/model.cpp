...
[[cpp11::register]]
SEXP dust_cpu_model_filter(SEXP ptr, SEXP step_end,
...
// Generated by odin.dust (version 0.2.21) - do not edit
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    for (int i = 1; i <= shared->dim_beta_1; ++i) {
      for (int j = 1; j <= shared->dim_beta_2; ++j) {
        internal.beta[i - 1 + shared->dim_beta_1 * (j - 1)] = (shared->beta_mode == 1 ? shared->beta_day[shared->dim_beta_day_1 * (i - 1) + step - 1] : ((shared->beta_mode == 2 ? beta_thresh * shared->rand_beta_factors[i - 1] : ((shared->beta_mode == 3 ? std::exp(log_beta) * shared->rand_beta_factors[i - 1] : ((shared->beta_mode == 4 ? beta_spont_behaviour[shared->dim_beta_spont_behaviour_1 * (j - 1) + i - 1] : ((shared->beta_mode == 5 ? beta_cut_peak * shared->rand_beta_factors[i - 1] : 0)))))))));
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_waning_1; ++i) {
      for (int j = 1; j <= shared->dim_n_waning_2; ++j) {
        internal.n_waning[i - 1 + shared->dim_n_waning_1 * (j - 1)] = (shared->include_waning == 1 ? (j != shared->n_vac ? dust::random::binomial<real_type>(rng_state, S[shared->dim_S_1 * (j - 1) + i - 1], shared->p_waning[shared->dim_p_waning_1 * (j - 1) + i - 1]) : (- odin_sum2<real_type>(internal.n_waning.data(), i - 1, i, 0, j - 1, shared->dim_n_waning_1))) : ((shared->include_waning == 2 ? (j != shared->n_vac ? internal.n_waning_tmp[shared->dim_n_waning_tmp_1 * (j - 1) + i - 1] - internal.n_waning_tmp[shared->dim_n_waning_tmp_1 * (j + 1 - 1) + i - 1] : (internal.n_waning_tmp[shared->dim_n_waning_tmp_1 * (j - 1) + i - 1])) : 0)));
      }
    }
