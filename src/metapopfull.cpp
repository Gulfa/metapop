...
[[cpp11::register]]
SEXP dust_cpu_metapopfull_alloc(cpp11::list r_pars, bool pars_multi, cpp11::sexp r_time,
...
[[cpp11::register]]
SEXP dust_cpu_metapopfull_update_state(SEXP ptr, SEXP r_pars, SEXP r_state,
...
[[cpp11::register]]
SEXP dust_cpu_metapopfull_filter(SEXP ptr, SEXP time_end,
...
public:
  ...
  metapopfull(const dust::pars_type<metapopfull>& pars) :
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    for (int i = 1; i <= shared->dim_n_RI_1; ++i) {
      for (int j = 1; j <= shared->dim_n_RI_2; ++j) {
        int k = 2;
        internal.n_RI[i - 1 + shared->dim_n_RI_1 * (j - 1) + shared->dim_n_RI_12 * (k - 1)] = dust::random::binomial<real_type>(rng_state, R[shared->dim_R_12 * 1 + shared->dim_R_1 * (j - 1) + i - 1], 1 - dust::math::exp(- odin_sum5<real_type>(internal.lambda_ij.data(), i - 1, i, j - 1, j, 0, shared->dim_lambda_ij_3, 0, shared->dim_lambda_ij_4, 0, 1, shared->dim_lambda_ij_1, shared->dim_lambda_ij_12, shared->dim_lambda_ij_123, shared->dim_lambda_ij_1234) * shared->cross_protection[shared->dim_cross_protection_1 * 0 + 1] * shared->dt));
      }
    }
    for (int i = 1; i <= shared->dim_n_RI_1; ++i) {
      for (int j = 1; j <= shared->dim_n_RI_2; ++j) {
        int k = 1;
        internal.n_RI[i - 1 + shared->dim_n_RI_1 * (j - 1) + shared->dim_n_RI_12 * (k - 1)] = dust::random::binomial<real_type>(rng_state, R[shared->dim_R_12 * 0 + shared->dim_R_1 * (j - 1) + i - 1], 1 - dust::math::exp(- odin_sum5<real_type>(internal.lambda_ij.data(), i - 1, i, j - 1, j, 0, shared->dim_lambda_ij_3, 0, shared->dim_lambda_ij_4, 0, 1, shared->dim_lambda_ij_1, shared->dim_lambda_ij_12, shared->dim_lambda_ij_123, shared->dim_lambda_ij_1234) * shared->cross_protection[shared->dim_cross_protection_1 * 1 + 0] * shared->dt));
      }
    }
    for (int i = 1; i <= shared->dim_p_SI_1; ++i) {
      for (int j = 1; j <= shared->dim_p_SI_2; ++j) {
        for (int k = 1; k <= shared->dim_p_SI_3; ++k) {
          internal.p_SI[i - 1 + shared->dim_p_SI_1 * (j - 1) + shared->dim_p_SI_12 * (k - 1)] = 1 - dust::math::exp(- odin_sum5<real_type>(internal.lambda_ij.data(), i - 1, i, j - 1, j, 0, shared->dim_lambda_ij_3, 0, shared->dim_lambda_ij_4, k - 1, k, shared->dim_lambda_ij_1, shared->dim_lambda_ij_12, shared->dim_lambda_ij_123, shared->dim_lambda_ij_1234) * shared->dt);
        }
      }
    }
...
namespace dust {
template<>
dust::pars_type<metapopfull> dust_pars<metapopfull>(cpp11::list user) {
  using real_type = typename metapopfull::real_type;
  auto shared = std::make_shared<metapopfull::shared_type>();
  metapopfull::internal_type internal;
  ...
  return dust::pars_type<metapopfull>(shared, internal);
}
template <>
cpp11::sexp dust_info<metapopfull>(const dust::pars_type<metapopfull>& pars) {
  const std::shared_ptr<const metapopfull::shared_type> shared = pars.shared;
...
cpp11::sexp dust_metapopfull_capabilities() {
  return dust::r::dust_capabilities<metapopfull>();
}

cpp11::sexp dust_metapopfull_gpu_info() {
  return dust::gpu::r::gpu_info();
}
using model_cpu = dust::dust_cpu<metapopfull>;

SEXP dust_cpu_metapopfull_alloc(cpp11::list r_pars, bool pars_multi, cpp11::sexp r_time,
                             cpp11::sexp r_n_particles, int n_threads,
                             cpp11::sexp r_seed, bool deterministic,
                             cpp11::sexp gpu_config, cpp11::sexp ode_control) {
  return dust::r::dust_cpu_alloc<metapopfull>(r_pars, pars_multi, r_time, r_n_particles,
...
SEXP dust_cpu_metapopfull_simulate(SEXP ptr, cpp11::sexp r_time_end) {
