...
// [[dust::param(beta_mode, has_default = FALSE, default_value = NULL, rank = 0, min = -Inf, max = Inf, integer = FALSE)]]
...
// [[dust::param(log_beta_ini, has_default = FALSE, default_value = NULL, rank = 0, min = -Inf, max = Inf, integer = FALSE)]]
...
public:
  ...
  struct shared_type {
    ...
    int dim_n_waning_1;
    int dim_n_waning_2;
    int dim_n_waning_tmp;
    int dim_n_waning_tmp_1;
    int dim_n_waning_tmp_2;
    int dim_p_HR;
    int dim_p_HR_1;
    int dim_p_HR_12;
    int dim_p_HR_2;
    ...
    real_type dt;
    std::vector<real_type> expected_health_loss;
    ...
    real_type initial_beta_cut_peak;
    std::vector<real_type> initial_beta_spont_behaviour;
    ...
    int offset_variable_hosp_inc;
    int offset_variable_incidence;
    int offset_variable_tmp;
  ...
  struct internal_type {
    ...
    std::vector<real_type> n_vac_help;
    std::vector<real_type> n_vac_now;
  ...
  size_t size() const {
    return shared->dim_A + shared->dim_B_D + shared->dim_B_D_H + shared->dim_B_D_ICU + shared->dim_D + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_ICU_P + shared->dim_ICU_R + shared->dim_N + shared->dim_P + shared->dim_R + shared->dim_S + shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + shared->dim_tot_hosp + shared->dim_tot_infected + shared->dim_tot_resp + shared->dim_tot_vac + shared->dim_tot_vac_adm + 13;
  }
  std::vector<real_type> initial(size_t step, rng_state_type& rng_state) {
    ...
    state[6] = shared->initial_peak_timer;
    state[7] = shared->initial_sum_help_vac;
    state[8] = shared->initial_log_beta;
    state[9] = shared->initial_log_beta_diff_hist;
    state[10] = shared->initial_beta_thresh;
    state[11] = shared->initial_tot_N;
    state[12] = shared->initial_tot_hosp_inc;
    std::copy(shared->initial_sum_vac.begin(), shared->initial_sum_vac.end(), state.begin() + 13);
    std::copy(shared->initial_hosp_inc.begin(), shared->initial_hosp_inc.end(), state.begin() + shared->offset_variable_hosp_inc);
    ...
    std::copy(shared->initial_tmp.begin(), shared->initial_tmp.end(), state.begin() + shared->offset_variable_tmp);
    ...
    std::copy(shared->initial_tot_vac_adm.begin(), shared->initial_tot_vac_adm.end(), state.begin() + shared->offset_variable_tot_vac_adm);
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    const real_type beta_thresh = state[10];
    const real_type log_beta = state[8];
    const real_type * contact_change = state + shared->offset_variable_contact_change;
    const real_type * beta_spont_behaviour = state + shared->offset_variable_beta_spont_behaviour;
    const real_type beta_cut_peak = state[1];
    ...
    const real_type * hosp_inc = state + shared->offset_variable_hosp_inc;
    const real_type * incidence = state + shared->offset_variable_incidence;
    const real_type tot_hosp_inc = state[12];
    ...
    const real_type * tot_vac_adm = state + shared->offset_variable_tot_vac_adm;
    ...
    real_type log_beta_diff = (fmodr<real_type>(step, (shared->rand_beta_days * shared->steps_per_day)) == (shared->rand_beta_days * shared->steps_per_day) - 1 ? dust::random::normal<real_type>(rng_state, 0, shared->rand_beta_sd) : 0);
    ...
    state_next[11] = odin_sum2<real_type>(N, 0, shared->dim_N_1, 0, shared->dim_N_2, shared->dim_N_1);
    ...
    for (int i = 1; i <= shared->dim_beta_1; ++i) {
      for (int j = 1; j <= shared->dim_beta_2; ++j) {
        internal.beta[i - 1 + shared->dim_beta_1 * (j - 1)] = (shared->beta_mode == 1 ? shared->beta_day[shared->dim_beta_day_1 * (i - 1) + step - 1] : ((shared->beta_mode == 2 ? beta_thresh * shared->rand_beta_factors[i - 1] : ((shared->beta_mode == 3 ? dust::math::exp(log_beta) * shared->rand_beta_factors[i - 1] : ((shared->beta_mode == 4 ? beta_spont_behaviour[shared->dim_beta_spont_behaviour_1 * (j - 1) + i - 1] : ((shared->beta_mode == 5 ? beta_cut_peak * shared->rand_beta_factors[i - 1] : ((shared->beta_mode == 6 ? shared->beta_day[shared->dim_beta_day_1 * (i - 1) + step - 1] * dust::math::exp(log_beta) : 0)))))))))));
      }
    }
    for (int i = 1; i <= shared->dim_beta_reduction_1; ++i) {
      for (int j = 1; j <= shared->dim_beta_reduction_2; ++j) {
        internal.beta_reduction[i - 1 + shared->dim_beta_reduction_1 * (j - 1)] = 1 - shared->beta_day[shared->dim_beta_day_1 * (i - 1) + step - 1] / (real_type) shared->spont_behav_change_params[0];
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_vac_help_1; ++i) {
      int j = 1;
      internal.n_vac_help[i - 1 + shared->dim_n_vac_help_1 * (j - 1)] = (shared->vax_type == 1 || shared->n_vac == 1 ? 0 : (dust::random::binomial<real_type>(rng_state, internal.vax_time_step[shared->dim_vax_time_step_1 * 0 + i - 1], internal.tmp_div_vac[shared->dim_tmp_div_vac_1 * 0 + i - 1] / (real_type) odin_sum2<real_type>(internal.tmp_div_vac.data(), i - 1, i, 0, shared->dim_tmp_div_vac_2, shared->dim_tmp_div_vac_1))));
    }
    for (int i = 1; i <= shared->dim_n_vac_help_1; ++i) {
      for (int j = 2; j <= shared->n_vac; ++j) {
        internal.n_vac_help[i - 1 + shared->dim_n_vac_help_1 * (j - 1)] = (shared->vax_type == 1 || shared->n_vac == 1 || j == shared->n_vac ? 0 : ((internal.vax_time_step[shared->dim_vax_time_step_1 * 0 + i - 1] - odin_sum2<real_type>(internal.n_vac_help.data(), i - 1, i, 0, j - 1, shared->dim_n_vac_help_1) > 0 && j <= shared->vac_struct_length ? dust::random::binomial<real_type>(rng_state, internal.vax_time_step[shared->dim_vax_time_step_1 * 0 + i - 1] - odin_sum2<real_type>(internal.n_vac_help.data(), i - 1, i, 0, j - 1, shared->dim_n_vac_help_1), internal.tmp_div_vac[shared->dim_tmp_div_vac_1 * (j - 1) + i - 1] / (real_type) odin_sum2<real_type>(internal.tmp_div_vac.data(), i - 1, i, j - 1, shared->n_vac, shared->dim_tmp_div_vac_1)) : (0))));
      }
    }
    ...
    for (int i = 1; i <= shared->dim_tot_vac_adm_1; ++i) {
      for (int j = 1; j <= shared->dim_tot_vac_adm_2; ++j) {
        state_next[shared->offset_variable_tot_vac_adm + i - 1 + shared->dim_tot_vac_adm_1 * (j - 1)] = tot_vac_adm[shared->dim_tot_vac_adm_1 * (j - 1) + i - 1] + internal.vax_time_step[shared->dim_vax_time_step_1 * (j - 1) + i - 1];
      }
    }
    for (int i = 1; i <= shared->dim_waning_tmp_1; ++i) {
      for (int j = 1; j <= shared->dim_waning_tmp_2; ++j) {
        internal.waning_tmp[i - 1 + shared->dim_waning_tmp_1 * (j - 1)] = (shared->vac_struct_length == 0 ? odin_sum3<real_type>(internal.n_RS.data(), i - 1, i, j - 1, j, 0, shared->dim_n_RS_3, shared->dim_n_RS_1, shared->dim_n_RS_12) : ((fmodr<real_type>(j, shared->vac_struct_length) == 0 ? odin_sum3<real_type>(internal.n_RS.data(), i - 1, i, 0, shared->dim_n_RS_2, shared->index[j - 1] - 1, shared->index[j - 1], shared->dim_n_RS_1, shared->dim_n_RS_12) : 0)));
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_RI_1; ++i) {
      for (int j = 1; j <= shared->dim_n_RI_2; ++j) {
        for (int k = 1; k <= shared->dim_n_RI_3; ++k) {
          internal.n_RI[i - 1 + shared->dim_n_RI_1 * (j - 1) + shared->dim_n_RI_12 * (k - 1)] = (shared->n_strain == 1 ? 0 : ((k == 1 ? dust::random::binomial<real_type>(rng_state, R[shared->dim_R_12 * 0 + shared->dim_R_1 * (j - 1) + i - 1], 1 - dust::math::exp(- odin_sum5<real_type>(internal.lambda_ij.data(), i - 1, i, j - 1, j, 0, shared->dim_lambda_ij_3, 0, shared->dim_lambda_ij_4, 0, 1, shared->dim_lambda_ij_1, shared->dim_lambda_ij_12, shared->dim_lambda_ij_123, shared->dim_lambda_ij_1234) * shared->cross_protection[shared->dim_cross_protection_1 * 1 + 0] * shared->symp_asymp_effect[shared->dim_symp_asymp_effect_12 * (k - 1) + shared->dim_symp_asymp_effect_1 * (j - 1) + i - 1] * shared->dt)) : ((k == 2 ? dust::random::binomial<real_type>(rng_state, R[shared->dim_R_12 * 1 + shared->dim_R_1 * (j - 1) + i - 1], 1 - dust::math::exp(- odin_sum5<real_type>(internal.lambda_ij.data(), i - 1, i, j - 1, j, 0, shared->dim_lambda_ij_3, 0, shared->dim_lambda_ij_4, 0, 1, shared->dim_lambda_ij_1, shared->dim_lambda_ij_12, shared->dim_lambda_ij_123, shared->dim_lambda_ij_1234) * shared->cross_protection[shared->dim_cross_protection_1 * 0 + 1] * shared->symp_asymp_effect[shared->dim_symp_asymp_effect_12 * (k - 1) + shared->dim_symp_asymp_effect_1 * (j - 1) + i - 1] * shared->dt)) : 0)))));
        }
      }
    }
    ...
    state_next[7] = odin_sum2<real_type>(internal.n_vac_help.data(), 0, shared->dim_n_vac_help_1, 0, shared->dim_n_vac_help_2, shared->dim_n_vac_help_1);
    for (int i = 1; i <= shared->dim_tmp_1; ++i) {
      for (int j = 1; j <= shared->dim_tmp_2; ++j) {
        state_next[shared->offset_variable_tmp + i - 1 + shared->dim_tmp_1 * (j - 1)] = internal.n_vac_help[shared->dim_n_vac_help_1 * (j - 1) + i - 1];
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_SE_1; ++i) {
      for (int j = 1; j <= shared->dim_n_SE_2; ++j) {
        for (int k = 1; k <= shared->dim_n_SE_3; ++k) {
          internal.n_SE[i - 1 + shared->dim_n_SE_1 * (j - 1) + shared->dim_n_SE_12 * (k - 1)] = (shared->n_strain == 1 ? internal.n_SE_tot[shared->dim_n_SE_tot_1 * (j - 1) + i - 1] : ((shared->n_strain == 2 ? (k == 1 ? dust::random::binomial<real_type>(rng_state, internal.n_SE_tot[shared->dim_n_SE_tot_1 * (j - 1) + i - 1], internal.rel_strain[shared->dim_rel_strain_12 * 0 + shared->dim_rel_strain_1 * (j - 1) + i - 1]) : (internal.n_SE_tot[shared->dim_n_SE_tot_1 * (j - 1) + i - 1] - internal.n_SE[shared->dim_n_SE_12 * 0 + shared->dim_n_SE_1 * (j - 1) + i - 1])) : 0)));
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_contact_change_1; ++i) {
      for (int j = 1; j <= shared->dim_contact_change_2; ++j) {
        state_next[shared->offset_variable_contact_change + i - 1 + shared->dim_contact_change_1 * (j - 1)] = (internal.kernel[shared->dim_kernel_1 * (j - 1) + i - 1] > 15 ? 1 : (dust::math::exp(internal.kernel[shared->dim_kernel_1 * (j - 1) + i - 1]) / (real_type) (1 + dust::math::exp(internal.kernel[shared->dim_kernel_1 * (j - 1) + i - 1]))));
      }
    }
    ...
    for (int i = 1; i <= shared->dim_hosp_inc; ++i) {
      state_next[shared->offset_variable_hosp_inc + i - 1] = odin_sum3<real_type>(internal.n_IH.data(), i - 1, i, 0, shared->dim_n_IH_2, 0, shared->dim_n_IH_3, shared->dim_n_IH_1, shared->dim_n_IH_12) + odin_sum3<real_type>(internal.n_IICU.data(), i - 1, i, 0, shared->dim_n_IICU_2, 0, shared->dim_n_IICU_3, shared->dim_n_IICU_1, shared->dim_n_IICU_12);
    }
    ...
    state_next[12] = (fmodr<real_type>(step, (shared->incidence_steps_measurement * shared->steps_per_day)) == 0 ? odin_sum3<real_type>(internal.n_IH.data(), 0, shared->dim_n_IH_1, 0, shared->dim_n_IH_2, 0, shared->dim_n_IH_3, shared->dim_n_IH_1, shared->dim_n_IH_12) + odin_sum3<real_type>(internal.n_IICU.data(), 0, shared->dim_n_IICU_1, 0, shared->dim_n_IICU_2, 0, shared->dim_n_IICU_3, shared->dim_n_IICU_1, shared->dim_n_IICU_12) : tot_hosp_inc + odin_sum3<real_type>(internal.n_IH.data(), 0, shared->dim_n_IH_1, 0, shared->dim_n_IH_2, 0, shared->dim_n_IH_3, shared->dim_n_IH_1, shared->dim_n_IH_12) + odin_sum3<real_type>(internal.n_IICU.data(), 0, shared->dim_n_IICU_1, 0, shared->dim_n_IICU_2, 0, shared->dim_n_IICU_3, shared->dim_n_IICU_1, shared->dim_n_IICU_12));
    ...
    for (int i = 1; i <= shared->dim_n_vac_now_1; ++i) {
      for (int j = 1; j <= shared->dim_n_vac_now_2; ++j) {
        internal.n_vac_now[i - 1 + shared->dim_n_vac_now_1 * (j - 1)] = (shared->vax_type == 1 ? (- internal.vax_time_step[shared->dim_vax_time_step_1 * (j - 1) + i - 1] + odin_sum3<real_type>(internal.n_SE.data(), i - 1, i, j - 1, j, 0, shared->dim_n_SE_3, shared->dim_n_SE_1, shared->dim_n_SE_12) > S[shared->dim_S_1 * (j - 1) + i - 1] ? - S[shared->dim_S_1 * (j - 1) + i - 1] + odin_sum3<real_type>(internal.n_SE.data(), i - 1, i, j - 1, j, 0, shared->dim_n_SE_3, shared->dim_n_SE_1, shared->dim_n_SE_12) : dust::math::round(internal.vax_time_step[shared->dim_vax_time_step_1 * (j - 1) + i - 1] * S[shared->dim_S_1 * 0 + i - 1] / (real_type) N[shared->dim_N_1 * 0 + i - 1])) : ((j < shared->vac_struct_length ? (internal.n_vac_help[shared->dim_n_vac_help_1 * (j - 1) + i - 1] > S[shared->dim_S_1 * (j - 1) + i - 1] - odin_sum3<real_type>(internal.n_SE.data(), i - 1, i, j - 1, j, 0, shared->dim_n_SE_3, shared->dim_n_SE_1, shared->dim_n_SE_12) ? - (S[shared->dim_S_1 * (j - 1) + i - 1] - odin_sum3<real_type>(internal.n_SE.data(), i - 1, i, j - 1, j, 0, shared->dim_n_SE_3, shared->dim_n_SE_1, shared->dim_n_SE_12)) : (- internal.n_vac_help[shared->dim_n_vac_help_1 * (j - 1) + i - 1])) : ((j == shared->vac_struct_length ? - odin_sum2<real_type>(internal.n_vac_now.data(), i - 1, i, 0, shared->vac_struct_length - 1, shared->dim_n_vac_now_1) : (0))))));
      }
    }
    ...
    for (int i = 1; i <= shared->dim_sum_vac; ++i) {
      state_next[13 + i - 1] = odin_sum2<real_type>(internal.n_vac_now.data(), 0, shared->dim_n_vac_now_1, i - 1, i, shared->dim_n_vac_now_1);
    }
...
dust::pars_type<model> dust_pars<model>(cpp11::list user) {
  ...
  shared->initial_log_beta_diff_hist = 0;
  shared->initial_peak_timer = 0;
  shared->initial_peak_trigger = 0;
  shared->initial_sum_help_vac = 0;
  ...
  shared->rand_beta_sd = static_cast<real_type>(0.10000000000000001);
  ...
  shared->N_steps = user_get_scalar<int>(user, "N_steps", shared->N_steps, NA_INTEGER, NA_INTEGER);
  ...
  shared->log_beta_ini = user_get_scalar<real_type>(user, "log_beta_ini", shared->log_beta_ini, NA_REAL, NA_REAL);
  shared->n = user_get_scalar<int>(user, "n", shared->n, NA_INTEGER, NA_INTEGER);
  shared->n_strain = user_get_scalar<int>(user, "n_strain", shared->n_strain, NA_INTEGER, NA_INTEGER);
  shared->n_vac = user_get_scalar<int>(user, "n_vac", shared->n_vac, NA_INTEGER, NA_INTEGER);
  ...
  shared->dim_N_imp_non_zero_1 = shared->n;
  shared->dim_N_imp_non_zero_2 = shared->n_vac;
  shared->dim_N_imp_non_zero_3 = shared->n_strain;
  ...
  shared->dim_beta_1 = shared->n;
  shared->dim_beta_2 = shared->n_vac;
  shared->dim_beta_day_1 = shared->N_steps;
  shared->dim_beta_day_2 = shared->n;
  shared->dim_beta_norm = shared->n;
  shared->dim_beta_reduction_1 = shared->n;
  shared->dim_beta_reduction_2 = shared->n_vac;
  shared->dim_beta_spont_behaviour_1 = shared->n;
  shared->dim_beta_spont_behaviour_2 = shared->n_vac;
  shared->dim_beta_strain = shared->n_strain;
  shared->dim_contact_change_1 = shared->n;
  shared->dim_contact_change_2 = shared->n_vac;
  shared->dim_cross_protection_1 = shared->n_strain;
  shared->dim_cross_protection_2 = shared->n_strain;
  shared->dim_expected_health_loss_1 = shared->n;
  shared->dim_expected_health_loss_2 = shared->n_vac;
  ...
  shared->dim_n_vac_help_1 = shared->n;
  shared->dim_n_vac_help_2 = shared->n_vac;
  shared->dim_n_vac_now_1 = shared->n;
  shared->dim_n_vac_now_2 = shared->n_vac;
  shared->dim_n_waning_1 = shared->n;
  shared->dim_n_waning_2 = shared->n_vac;
  shared->dim_n_waning_tmp_1 = shared->n;
  shared->dim_n_waning_tmp_2 = shared->n_vac;
  ...
  shared->dim_sum_vac = shared->n_vac;
  ...
  shared->dim_tmp_1 = shared->n;
  shared->dim_tmp_2 = shared->n_vac;
  ...
  shared->dim_tot_vac_adm_1 = shared->n;
  shared->dim_tot_vac_adm_2 = shared->n_vac;
  shared->dim_tot_vac_adm_ini_1 = shared->n;
  shared->dim_tot_vac_adm_ini_2 = shared->n_vac;
  ...
  shared->dim_waning_tmp_1 = shared->n;
  shared->dim_waning_tmp_2 = shared->n_vac;
  shared->initial_beta_thresh = shared->threshold_ini;
  shared->initial_log_beta = shared->log_beta_ini;
  shared->p_D = 1 - dust::math::exp(- shared->dt / (real_type) shared->time_before_death);
  shared->p_D_H = 1 - dust::math::exp(- shared->dt / (real_type) shared->time_before_death_hosp);
  shared->p_D_ICU = 1 - dust::math::exp(- shared->dt / (real_type) shared->time_before_death_icu);
  shared->p_EI = 1 - dust::math::exp(- shared->dt / (real_type) shared->latent_period);
  shared->p_IR = 1 - dust::math::exp(- shared->dt / (real_type) shared->infectious_period);
  shared->p_PI = 1 - dust::math::exp(- shared->dt / (real_type) shared->pre_sympt_period);
  shared->p_waning_inf = 1 - dust::math::exp(- shared->dt / (real_type) shared->waning_inf);
  shared->spont_behav_change_params = user_get_array_fixed<real_type, 1>(user, "spont_behav_change_params", shared->spont_behav_change_params, {shared->dim_spont_behav_change_params}, NA_REAL, NA_REAL);
  ...
  shared->index = std::vector<real_type>(shared->dim_index);
  ...
  shared->initial_sum_vac = std::vector<real_type>(shared->dim_sum_vac);
  ...
  shared->dim_N_imp_non_zero = shared->dim_N_imp_non_zero_1 * shared->dim_N_imp_non_zero_2 * shared->dim_N_imp_non_zero_3;
  shared->dim_N_imp_non_zero_12 = shared->dim_N_imp_non_zero_1 * shared->dim_N_imp_non_zero_2;
  ...
  shared->dim_beta = shared->dim_beta_1 * shared->dim_beta_2;
  shared->dim_beta_day = shared->dim_beta_day_1 * shared->dim_beta_day_2;
  shared->dim_beta_reduction = shared->dim_beta_reduction_1 * shared->dim_beta_reduction_2;
  shared->dim_beta_spont_behaviour = shared->dim_beta_spont_behaviour_1 * shared->dim_beta_spont_behaviour_2;
  shared->dim_contact_change = shared->dim_contact_change_1 * shared->dim_contact_change_2;
  shared->dim_cross_protection = shared->dim_cross_protection_1 * shared->dim_cross_protection_2;
  shared->dim_expected_health_loss = shared->dim_expected_health_loss_1 * shared->dim_expected_health_loss_2;
  ...
  shared->dim_n_vac_help = shared->dim_n_vac_help_1 * shared->dim_n_vac_help_2;
  shared->dim_n_vac_now = shared->dim_n_vac_now_1 * shared->dim_n_vac_now_2;
  shared->dim_n_waning = shared->dim_n_waning_1 * shared->dim_n_waning_2;
  shared->dim_n_waning_tmp = shared->dim_n_waning_tmp_1 * shared->dim_n_waning_tmp_2;
  ...
  shared->dim_tmp = shared->dim_tmp_1 * shared->dim_tmp_2;
  ...
  shared->dim_tot_vac_adm = shared->dim_tot_vac_adm_1 * shared->dim_tot_vac_adm_2;
  shared->dim_tot_vac_adm_ini = shared->dim_tot_vac_adm_ini_1 * shared->dim_tot_vac_adm_ini_2;
  ...
  shared->dim_waning_tmp = shared->dim_waning_tmp_1 * shared->dim_waning_tmp_2;
  shared->initial_beta_cut_peak = shared->beta_cut_peak_param[0];
  for (int i = 1; i <= shared->dim_hosp_inc; ++i) {
  ...
  for (int i = 1; i <= shared->dim_sum_vac; ++i) {
    shared->initial_sum_vac[i - 1] = 0;
  }
  shared->offset_variable_contact_change = shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + 13;
  shared->offset_variable_hosp_inc = shared->dim_sum_vac + 13;
  shared->offset_variable_incidence = shared->dim_hosp_inc + shared->dim_sum_vac + 13;
  ...
  internal.N_imp_non_zero = std::vector<real_type>(shared->dim_N_imp_non_zero);
  internal.beta = std::vector<real_type>(shared->dim_beta);
  internal.beta_reduction = std::vector<real_type>(shared->dim_beta_reduction);
  ...
  shared->initial_beta_spont_behaviour = std::vector<real_type>(shared->dim_beta_spont_behaviour);
  shared->initial_contact_change = std::vector<real_type>(shared->dim_contact_change);
  shared->initial_tmp = std::vector<real_type>(shared->dim_tmp);
  ...
  shared->initial_tot_vac_adm = std::vector<real_type>(shared->dim_tot_vac_adm);
  ...
  internal.n_vac_help = std::vector<real_type>(shared->dim_n_vac_help);
  internal.n_vac_now = std::vector<real_type>(shared->dim_n_vac_now);
  internal.n_waning = std::vector<real_type>(shared->dim_n_waning);
  internal.n_waning_tmp = std::vector<real_type>(shared->dim_n_waning_tmp);
  ...
  shared->expected_health_loss = user_get_array_fixed<real_type, 2>(user, "expected_health_loss", shared->expected_health_loss, {shared->dim_expected_health_loss_1, shared->dim_expected_health_loss_2}, NA_REAL, NA_REAL);
  ...
  for (int i = 1; i <= shared->dim_index; ++i) {
    shared->index[i - 1] = static_cast<int>((i - 1) / (real_type) shared->vac_struct_length) + 1;
  }
  for (int i = 1; i <= shared->dim_contact_change_1; ++i) {
    for (int j = 1; j <= shared->dim_contact_change_2; ++j) {
      shared->initial_contact_change[i - 1 + shared->dim_contact_change_1 * (j - 1)] = 0;
    }
  }
  for (int i = 1; i <= shared->dim_tmp_1; ++i) {
    for (int j = 1; j <= shared->dim_tmp_2; ++j) {
      shared->initial_tmp[i - 1 + shared->dim_tmp_1 * (j - 1)] = 0;
    }
  }
  ...
  shared->offset_variable_A = shared->dim_Ea + shared->dim_Es + shared->dim_I + shared->dim_N + shared->dim_P + shared->dim_S + shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + shared->dim_tot_vac + shared->dim_tot_vac_adm + 13;
  shared->offset_variable_B_D = shared->dim_A + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_ICU_P + shared->dim_ICU_R + shared->dim_N + shared->dim_P + shared->dim_S + shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + shared->dim_tot_vac + shared->dim_tot_vac_adm + 13;
  ...
  shared->offset_variable_D = shared->dim_A + shared->dim_B_D + shared->dim_B_D_H + shared->dim_B_D_ICU + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_ICU_P + shared->dim_ICU_R + shared->dim_N + shared->dim_P + shared->dim_R + shared->dim_S + shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + shared->dim_tot_vac + shared->dim_tot_vac_adm + 13;
  shared->offset_variable_Ea = shared->dim_N + shared->dim_S + shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + shared->dim_tot_vac + shared->dim_tot_vac_adm + 13;
  shared->offset_variable_Es = shared->dim_Ea + shared->dim_N + shared->dim_S + shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + shared->dim_tot_vac + shared->dim_tot_vac_adm + 13;
  shared->offset_variable_H = shared->dim_A + shared->dim_Ea + shared->dim_Es + shared->dim_I + shared->dim_N + shared->dim_P + shared->dim_S + shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + shared->dim_tot_vac + shared->dim_tot_vac_adm + 13;
  shared->offset_variable_I = shared->dim_Ea + shared->dim_Es + shared->dim_N + shared->dim_P + shared->dim_S + shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + shared->dim_tot_vac + shared->dim_tot_vac_adm + 13;
  shared->offset_variable_ICU_H = shared->dim_A + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_N + shared->dim_P + shared->dim_S + shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + shared->dim_tot_vac + shared->dim_tot_vac_adm + 13;
  shared->offset_variable_ICU_P = shared->dim_A + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_ICU_R + shared->dim_N + shared->dim_P + shared->dim_S + shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + shared->dim_tot_vac + shared->dim_tot_vac_adm + 13;
  shared->offset_variable_ICU_R = shared->dim_A + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_N + shared->dim_P + shared->dim_S + shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + shared->dim_tot_vac + shared->dim_tot_vac_adm + 13;
  shared->offset_variable_N = shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + 13;
  shared->offset_variable_P = shared->dim_Ea + shared->dim_Es + shared->dim_N + shared->dim_S + shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + shared->dim_tot_vac + shared->dim_tot_vac_adm + 13;
  ...
  shared->offset_variable_S = shared->dim_N + shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + 13;
  shared->offset_variable_beta_spont_behaviour = shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + 13;
  shared->offset_variable_tmp = shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + 13;
  shared->offset_variable_tot_hosp = shared->dim_A + shared->dim_B_D + shared->dim_B_D_H + shared->dim_B_D_ICU + shared->dim_D + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_ICU_P + shared->dim_ICU_R + shared->dim_N + shared->dim_P + shared->dim_R + shared->dim_S + shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + shared->dim_tot_infected + shared->dim_tot_vac + shared->dim_tot_vac_adm + 13;
  ...
  shared->offset_variable_tot_resp = shared->dim_A + shared->dim_B_D + shared->dim_B_D_H + shared->dim_B_D_ICU + shared->dim_D + shared->dim_Ea + shared->dim_Es + shared->dim_H + shared->dim_I + shared->dim_ICU_H + shared->dim_ICU_P + shared->dim_ICU_R + shared->dim_N + shared->dim_P + shared->dim_R + shared->dim_S + shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + shared->dim_tot_hosp + shared->dim_tot_infected + shared->dim_tot_vac + shared->dim_tot_vac_adm + 13;
  shared->offset_variable_tot_vac = shared->dim_N + shared->dim_S + shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + 13;
  shared->offset_variable_tot_vac_adm = shared->dim_N + shared->dim_S + shared->dim_beta_spont_behaviour + shared->dim_contact_change + shared->dim_hosp_inc + shared->dim_incidence + shared->dim_sum_vac + shared->dim_tmp + shared->dim_tot_vac + 13;
  ...
  shared->tot_vac_adm_ini = user_get_array_fixed<real_type, 2>(user, "tot_vac_adm_ini", shared->tot_vac_adm_ini, {shared->dim_tot_vac_adm_ini_1, shared->dim_tot_vac_adm_ini_2}, NA_REAL, NA_REAL);
  ...
  for (int i = 1; i <= shared->dim_N_1; ++i) {
    for (int j = 1; j <= shared->dim_N_2; ++j) {
      shared->initial_N[i - 1 + shared->dim_N_1 * (j - 1)] = shared->S_ini[shared->dim_S_ini_1 * (j - 1) + i - 1] + odin_sum3<real_type>(shared->Ea_ini.data(), i - 1, i, j - 1, j, 0, shared->dim_Ea_ini_3, shared->dim_Ea_ini_1, shared->dim_Ea_ini_12) + odin_sum3<real_type>(shared->Es_ini.data(), i - 1, i, j - 1, j, 0, shared->dim_Es_ini_3, shared->dim_Es_ini_1, shared->dim_Es_ini_12) + odin_sum3<real_type>(shared->P_ini.data(), i - 1, i, j - 1, j, 0, shared->dim_P_ini_3, shared->dim_P_ini_1, shared->dim_P_ini_12) + odin_sum3<real_type>(shared->I_ini.data(), i - 1, i, j - 1, j, 0, shared->dim_I_ini_3, shared->dim_I_ini_1, shared->dim_I_ini_12) + odin_sum3<real_type>(shared->A_ini.data(), i - 1, i, j - 1, j, 0, shared->dim_A_ini_3, shared->dim_A_ini_1, shared->dim_A_ini_12) + odin_sum3<real_type>(shared->H_ini.data(), i - 1, i, j - 1, j, 0, shared->dim_H_ini_3, shared->dim_H_ini_1, shared->dim_H_ini_12) + odin_sum3<real_type>(shared->ICU_H_ini.data(), i - 1, i, j - 1, j, 0, shared->dim_ICU_H_ini_3, shared->dim_ICU_H_ini_1, shared->dim_ICU_H_ini_12) + odin_sum3<real_type>(shared->ICU_R_ini.data(), i - 1, i, j - 1, j, 0, shared->dim_ICU_R_ini_3, shared->dim_ICU_R_ini_1, shared->dim_ICU_R_ini_12) + odin_sum3<real_type>(shared->ICU_P_ini.data(), i - 1, i, j - 1, j, 0, shared->dim_ICU_P_ini_3, shared->dim_ICU_P_ini_1, shared->dim_ICU_P_ini_12) + odin_sum3<real_type>(shared->B_D_ini.data(), i - 1, i, j - 1, j, 0, shared->dim_B_D_ini_3, shared->dim_B_D_ini_1, shared->dim_B_D_ini_12) + odin_sum3<real_type>(shared->B_D_ICU_ini.data(), i - 1, i, j - 1, j, 0, shared->dim_B_D_ICU_ini_3, shared->dim_B_D_ICU_ini_1, shared->dim_B_D_ICU_ini_12) + odin_sum3<real_type>(shared->B_D_H_ini.data(), i - 1, i, j - 1, j, 0, shared->dim_B_D_H_ini_3, shared->dim_B_D_H_ini_1, shared->dim_B_D_H_ini_12) + odin_sum3<real_type>(shared->R_ini.data(), i - 1, i, j - 1, j, 0, shared->dim_R_ini_3, shared->dim_R_ini_1, shared->dim_R_ini_12) + odin_sum3<real_type>(shared->D_ini.data(), i - 1, i, j - 1, j, 0, shared->dim_D_ini_3, shared->dim_D_ini_1, shared->dim_D_ini_12);
    }
  }
  for (int i = 1; i <= shared->dim_P_1; ++i) {
    for (int j = 1; j <= shared->dim_P_2; ++j) {
  ...
  for (int i = 1; i <= shared->dim_beta_spont_behaviour_1; ++i) {
    for (int j = 1; j <= shared->dim_beta_spont_behaviour_2; ++j) {
      shared->initial_beta_spont_behaviour[i - 1 + shared->dim_beta_spont_behaviour_1 * (j - 1)] = shared->beta_day[shared->dim_beta_day_1 * (i - 1) + 0];
    }
  }
  for (int i = 1; i <= shared->dim_tot_hosp_1; ++i) {
    for (int j = 1; j <= shared->dim_tot_hosp_2; ++j) {
  ...
  for (int i = 1; i <= shared->dim_tot_vac_adm_1; ++i) {
    for (int j = 1; j <= shared->dim_tot_vac_adm_2; ++j) {
      shared->initial_tot_vac_adm[i - 1 + shared->dim_tot_vac_adm_1 * (j - 1)] = shared->tot_vac_adm_ini[shared->dim_tot_vac_adm_ini_1 * (j - 1) + i - 1];
    }
  }
  for (int i = 1; i <= shared->dim_p_HR_1; ++i) {
    for (int j = 1; j <= shared->dim_p_HR_2; ++j) {
      for (int k = 1; k <= shared->dim_p_HR_3; ++k) {
        shared->p_HR[i - 1 + shared->dim_p_HR_1 * (j - 1) + shared->dim_p_HR_12 * (k - 1)] = 1 - dust::math::exp(- shared->dt / (real_type) shared->length_hosp[shared->dim_length_hosp_12 * (k - 1) + shared->dim_length_hosp_1 * (j - 1) + i - 1]);
      }
    }
  }
  for (int i = 1; i <= shared->dim_p_ICU_P_1; ++i) {
    for (int j = 1; j <= shared->dim_p_ICU_P_2; ++j) {
      for (int k = 1; k <= shared->dim_p_ICU_P_3; ++k) {
        shared->p_ICU_P[i - 1 + shared->dim_p_ICU_P_1 * (j - 1) + shared->dim_p_ICU_P_12 * (k - 1)] = 1 - dust::math::exp(- shared->dt / (real_type) shared->length_icu[shared->dim_length_icu_12 * (k - 1) + shared->dim_length_icu_1 * (j - 1) + i - 1]);
      }
    }
  }
  for (int i = 1; i <= shared->dim_p_ICU_PR_1; ++i) {
    for (int j = 1; j <= shared->dim_p_ICU_PR_2; ++j) {
      for (int k = 1; k <= shared->dim_p_ICU_PR_3; ++k) {
        shared->p_ICU_PR[i - 1 + shared->dim_p_ICU_PR_1 * (j - 1) + shared->dim_p_ICU_PR_12 * (k - 1)] = 1 - dust::math::exp(- shared->dt / (real_type) shared->post_icu[shared->dim_post_icu_12 * (k - 1) + shared->dim_post_icu_1 * (j - 1) + i - 1]);
      }
    }
  }
  for (int i = 1; i <= shared->dim_p_ICU_R_1; ++i) {
    for (int j = 1; j <= shared->dim_p_ICU_R_2; ++j) {
      for (int k = 1; k <= shared->dim_p_ICU_R_3; ++k) {
        shared->p_ICU_R[i - 1 + shared->dim_p_ICU_R_1 * (j - 1) + shared->dim_p_ICU_R_12 * (k - 1)] = 1 - dust::math::exp(- shared->dt / (real_type) shared->pre_icu[shared->dim_pre_icu_12 * (k - 1) + shared->dim_pre_icu_1 * (j - 1) + i - 1]);
      }
    }
  }
  ...
  for (int i = 1; i <= shared->dim_p_waning_1; ++i) {
    for (int j = 1; j <= shared->dim_p_waning_2; ++j) {
      shared->p_waning[i - 1 + shared->dim_p_waning_1 * (j - 1)] = 1 - dust::math::exp(- shared->dt / (real_type) shared->T_waning[shared->dim_T_waning_1 * (j - 1) + i - 1]);
    }
  }
...
cpp11::sexp dust_info<model>(const dust::pars_type<model>& pars) {
  ...
  cpp11::writable::strings nms({"time", "beta_cut_peak", "trigger", "incidence_int", "peak_trigger", "e_int", "peak_timer", "sum_help_vac", "log_beta", "log_beta_diff_hist", "beta_thresh", "tot_N", "tot_hosp_inc", "sum_vac", "hosp_inc", "incidence", "contact_change", "beta_spont_behaviour", "tmp", "N", "S", "tot_vac", "tot_vac_adm", "Ea", "Es", "P", "I", "A", "H", "ICU_H", "ICU_R", "ICU_P", "B_D", "B_D_H", "B_D_ICU", "R", "D", "tot_infected", "tot_hosp", "tot_resp"});
  cpp11::writable::list dim(40);
  ...
  dim[13] = cpp11::writable::integers({shared->dim_sum_vac});
  dim[14] = cpp11::writable::integers({shared->dim_hosp_inc});
  dim[15] = cpp11::writable::integers({shared->dim_incidence});
  dim[16] = cpp11::writable::integers({shared->dim_contact_change_1, shared->dim_contact_change_2});
  dim[17] = cpp11::writable::integers({shared->dim_beta_spont_behaviour_1, shared->dim_beta_spont_behaviour_2});
  dim[18] = cpp11::writable::integers({shared->dim_tmp_1, shared->dim_tmp_2});
  dim[19] = cpp11::writable::integers({shared->dim_N_1, shared->dim_N_2});
  dim[20] = cpp11::writable::integers({shared->dim_S_1, shared->dim_S_2});
  dim[21] = cpp11::writable::integers({shared->dim_tot_vac_1, shared->dim_tot_vac_2});
  dim[22] = cpp11::writable::integers({shared->dim_tot_vac_adm_1, shared->dim_tot_vac_adm_2});
  dim[23] = cpp11::writable::integers({shared->dim_Ea_1, shared->dim_Ea_2, shared->dim_Ea_3});
  dim[24] = cpp11::writable::integers({shared->dim_Es_1, shared->dim_Es_2, shared->dim_Es_3});
  dim[25] = cpp11::writable::integers({shared->dim_P_1, shared->dim_P_2, shared->dim_P_3});
  dim[26] = cpp11::writable::integers({shared->dim_I_1, shared->dim_I_2, shared->dim_I_3});
  dim[27] = cpp11::writable::integers({shared->dim_A_1, shared->dim_A_2, shared->dim_A_3});
  dim[28] = cpp11::writable::integers({shared->dim_H_1, shared->dim_H_2, shared->dim_H_3});
  dim[29] = cpp11::writable::integers({shared->dim_ICU_H_1, shared->dim_ICU_H_2, shared->dim_ICU_H_3});
  dim[30] = cpp11::writable::integers({shared->dim_ICU_R_1, shared->dim_ICU_R_2, shared->dim_ICU_R_3});
  dim[31] = cpp11::writable::integers({shared->dim_ICU_P_1, shared->dim_ICU_P_2, shared->dim_ICU_P_3});
  dim[32] = cpp11::writable::integers({shared->dim_B_D_1, shared->dim_B_D_2, shared->dim_B_D_3});
  dim[33] = cpp11::writable::integers({shared->dim_B_D_H_1, shared->dim_B_D_H_2, shared->dim_B_D_H_3});
  dim[34] = cpp11::writable::integers({shared->dim_B_D_ICU_1, shared->dim_B_D_ICU_2, shared->dim_B_D_ICU_3});
  dim[35] = cpp11::writable::integers({shared->dim_R_1, shared->dim_R_2, shared->dim_R_3});
  dim[36] = cpp11::writable::integers({shared->dim_D_1, shared->dim_D_2, shared->dim_D_3});
  dim[37] = cpp11::writable::integers({shared->dim_tot_infected_1, shared->dim_tot_infected_2, shared->dim_tot_infected_3});
  dim[38] = cpp11::writable::integers({shared->dim_tot_hosp_1, shared->dim_tot_hosp_2, shared->dim_tot_hosp_3});
  dim[39] = cpp11::writable::integers({shared->dim_tot_resp_1, shared->dim_tot_resp_2, shared->dim_tot_resp_3});
  dim.names() = nms;
  cpp11::writable::list index(40);
  ...
  index[13] = integer_sequence(14, shared->dim_sum_vac);
  index[14] = integer_sequence(shared->offset_variable_hosp_inc + 1, shared->dim_hosp_inc);
  index[15] = integer_sequence(shared->offset_variable_incidence + 1, shared->dim_incidence);
  index[16] = integer_sequence(shared->offset_variable_contact_change + 1, shared->dim_contact_change);
  index[17] = integer_sequence(shared->offset_variable_beta_spont_behaviour + 1, shared->dim_beta_spont_behaviour);
  index[18] = integer_sequence(shared->offset_variable_tmp + 1, shared->dim_tmp);
  index[19] = integer_sequence(shared->offset_variable_N + 1, shared->dim_N);
  index[20] = integer_sequence(shared->offset_variable_S + 1, shared->dim_S);
  index[21] = integer_sequence(shared->offset_variable_tot_vac + 1, shared->dim_tot_vac);
  index[22] = integer_sequence(shared->offset_variable_tot_vac_adm + 1, shared->dim_tot_vac_adm);
  index[23] = integer_sequence(shared->offset_variable_Ea + 1, shared->dim_Ea);
  index[24] = integer_sequence(shared->offset_variable_Es + 1, shared->dim_Es);
  index[25] = integer_sequence(shared->offset_variable_P + 1, shared->dim_P);
  index[26] = integer_sequence(shared->offset_variable_I + 1, shared->dim_I);
  index[27] = integer_sequence(shared->offset_variable_A + 1, shared->dim_A);
  index[28] = integer_sequence(shared->offset_variable_H + 1, shared->dim_H);
  index[29] = integer_sequence(shared->offset_variable_ICU_H + 1, shared->dim_ICU_H);
  index[30] = integer_sequence(shared->offset_variable_ICU_R + 1, shared->dim_ICU_R);
  index[31] = integer_sequence(shared->offset_variable_ICU_P + 1, shared->dim_ICU_P);
  index[32] = integer_sequence(shared->offset_variable_B_D + 1, shared->dim_B_D);
  index[33] = integer_sequence(shared->offset_variable_B_D_H + 1, shared->dim_B_D_H);
  index[34] = integer_sequence(shared->offset_variable_B_D_ICU + 1, shared->dim_B_D_ICU);
  index[35] = integer_sequence(shared->offset_variable_R + 1, shared->dim_R);
  index[36] = integer_sequence(shared->offset_variable_D + 1, shared->dim_D);
  index[37] = integer_sequence(shared->offset_variable_tot_infected + 1, shared->dim_tot_infected);
  index[38] = integer_sequence(shared->offset_variable_tot_hosp + 1, shared->dim_tot_hosp);
  index[39] = integer_sequence(shared->offset_variable_tot_resp + 1, shared->dim_tot_resp);
  index.names() = nms;
