...
n_vac_help[,1] <- if(vax_type ==1) 0 else (rbinom(vax_time_step[i,1], S[i,1]/sum(S[i,])))
vax_type <- user(1)
n_vac_help[,2:n_vac] <- if(vax_type==1) 0 else (rbinom(vax_time_step[i,1] - sum(n_vac_help[i, 1:(j - 1)]), S[i,j]/sum(S[i,j:n_vac])))
